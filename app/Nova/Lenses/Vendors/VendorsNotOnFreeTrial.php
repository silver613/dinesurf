...
class VendorsNotOnFreeTrial extends Lens
{
    /**
    ...
    public static function query(LensRequest $request, $query)
    {
        return $request->withOrdering($request->withFilters(
            $query->where('free_trial', false)->where('subscribed', false)->where('approved', true)
        ));
    }
    ...
    public function fields(NovaRequest $request)
    {
        return [
            ...
            Date::make('free_trial_start'),
            Date::make('free_trial_end'),
            Badge::make('type')->map([
                'restaurant' => 'warning',
            ])->hideFromIndex(),
            Boolean::make('open'),
    ...
    {
        return 'vendors-vendors-not-on-free-trial';
    }
}
