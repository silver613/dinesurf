<template>
  <div class="py-0 px-0 w-full">
    <div class="mb-6 w-full grid grid-cols-3  gap-3">
      <div
          class="h-40 md:w-[220px]   bg-grey  w-80 " v-for="(image, index) in images" :key="index" v-show="images.length > 0">
        <img
         
          :key="index"
          :src="image.src"
          class="block  imageCustom  "
          draggable="false"
    ...
    <!---->
    <div class="form-file mr-4">
      <form @submit.prevent="submit()">

        <div  class="relative  border border-dashed  border-dinesurf-green-400
        h-40 md:w-[220px]   w-80   flex items-center h-40  flex-col justify-center bg-gray-200 ">
